"""Animated GIF of posterior sampling a bimodal."""
...
def plot_marginals(x_lims, ou_dist, sde):
    ...
    axs[0].scatter(jnp.exp(hires_vals[0]), x_hires, c=hires_vals[0], edgecolor='None', cmap='magma', vmin=vmin, vmax=vmax)
    axs[1].imshow(vals.T, extent=(t_lims + x_lims), interpolation='bicubic', cmap='magma')
    axs[2].scatter(jnp.exp(hires_vals[-1]), x_hires, c=hires_vals[-1], edgecolor='None', cmap='magma', vmin=vmin, vmax=vmax)
